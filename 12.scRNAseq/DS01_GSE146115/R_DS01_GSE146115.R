setwd("F:/Ashley_Projects/Metabolism_Immune/12.scRNAseq/DS01_GSE146115/")
rm(list=ls())

library("ggplot2")
dat<-read.table("LIHC_GSE146115_CellMetainfo_table.tsv", sep = "\t", header = T)
dim(dat)
#[1] 3134   10
head(dat)
#############################malignancy##################################################
...
dat<-Read10X_h5("LIHC_GSE146115_expression.h5")
dim(dat)
#[1] 15527  3134
dat <- CreateSeuratObject(dat, project = "LIHC_DS1", min.cells = 3)
dim(dat)
#[1] 15527  3134
...
celltype_data<-read.table("LIHC_GSE146115_CellMetainfo_table.tsv", sep = "\t", header = T)
dim(RFS)
#[1] 3134     2
dim(celltype_data)
#[1] 3134    10
