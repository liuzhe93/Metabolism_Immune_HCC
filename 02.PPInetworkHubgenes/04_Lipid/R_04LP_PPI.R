setwd("F:/Ashley_Projects/Metabolism_Immune/02.PPInetworkHubgenes/04_Lipid/")
rm(list=ls())
...
write.csv(PPI_fil_highcon, "LP_input_highcon.csv", quote = F, row.names = F)

#####Cytoscape output
top10<-read.csv("LP_input_highcon.csv_MCC_top10 default node.csv")

top10_genes<-top10$name
#[1] "MED15" "MED17" "MED1"  "MED26" "CCNC"  "MED10" "MED31" "MED14" "MED4"  "MED28"
PPI_sel<-data.frame()
...
library(survival)
library(survminer)
genelist_lp<-read.csv("top10_withdegree.csv")
genelist_lp$Name
mydata_lp<-mydata[,c(genelist_lp$Name)]



library(corrplot)
M = cor(mydata_lp, use = "everything", method = "spearman")
testRes = cor.mtest(mydata_lp,  use = "everything", method = "spearman", conf.level = 0.95)
...
genelist_lp<-read.csv("top10_withdegree.csv")
genelist_lp$Name
mydata_lp<-mydata[,c(genelist_lp$Name, "Time", "Status")]
library("survival")
library("forestplot")
rt<-mydata_lp
