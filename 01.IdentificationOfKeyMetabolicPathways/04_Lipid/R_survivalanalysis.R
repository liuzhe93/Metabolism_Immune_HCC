...
gene_set<-read.csv("F:/Ashley_Projects/Metabolism_Immune/01.IdentificationOfKeyMetabolicPathways/04_Lipid/TableS1_Lipid.csv")
...
write.csv(ssgsea_scores,"F:/Ashley_Projects/Metabolism_Immune/01.IdentificationOfKeyMetabolicPathways/04_Lipid/noNormalize.csv",quote=F)

normalization<-function(x){
  return((x-min(x))/(max(x)-min(x)))}#设定normalization函数
nor_gsva_matrix1 <- normalization(ssgsea_scores)
write.csv(nor_gsva_matrix1,"F:/Ashley_Projects/Metabolism_Immune/01.IdentificationOfKeyMetabolicPathways/04_Lipid/Normalize.csv",quote=F)
...
#1st Qu.:0.6160   Class :character  
#Median :0.7094   Mode  :character  
#Mean   :0.6883                     
#3rd Qu.:0.7890                     
#Max.   :1.0000   
...
library(survival)
library(survminer)
res.cut<-surv_cutpoint(merged_cli_score,time="futime",event="fustat",variables="Lipid")
summary(res.cut)
#       cutpoint statistic
#Lipid 0.5282816  3.208198

pdf("F:/Ashley_Projects/Metabolism_Immune/01.IdentificationOfKeyMetabolicPathways/04_Lipid/Lipid_cutoff.pdf")
plot(res.cut,"Lipid",palette="npg")
dev.off()

res.cat<-surv_categorize(res.cut)

fit=survfit(Surv(futime, fustat) ~Lipid, data = res.cat)

head(res.cat)
diff<-survdiff(Surv(futime, fustat)~Lipid, data = res.cat)
...
outFile="F:/Ashley_Projects/Metabolism_Immune/01.IdentificationOfKeyMetabolicPathways/04_Lipid/Lipid_survival.pdf"
