...
gene_set<-read.csv("F:/Ashley_Projects/Metabolism_Immune/01.IdentificationOfKeyMetabolicPathways/02_Carbohydrate/TableS1_Carbohydrate.csv")
...
write.csv(ssgsea_scores,"F:/Ashley_Projects/Metabolism_Immune/01.IdentificationOfKeyMetabolicPathways/02_Carbohydrate/noNormalize.csv",quote=F)

normalization<-function(x){
  return((x-min(x))/(max(x)-min(x)))}#设定normalization函数
nor_gsva_matrix1 <- normalization(ssgsea_scores)
write.csv(nor_gsva_matrix1,"F:/Ashley_Projects/Metabolism_Immune/01.IdentificationOfKeyMetabolicPathways/02_Carbohydrate/Normalize.csv",quote=F)
...
colnames(pathway_data)<-"Carbohydrate"
...
#1st Qu.:0.4289   Class :character  
#Median :0.5462   Mode  :character  
#Mean   :0.5437                     
#3rd Qu.:0.6895                     
#Max.   :1.0000
...
library(survival)
library(survminer)
res.cut<-surv_cutpoint(merged_cli_score,time="futime",event="fustat",variables="Carbohydrate")
summary(res.cut)
#              cutpoint statistic
#Carbohydrate 0.5429155   2.57289

pdf("F:/Ashley_Projects/Metabolism_Immune/01.IdentificationOfKeyMetabolicPathways/02_Carbohydrate/Carbohydrate_cutoff.pdf")
plot(res.cut,"Carbohydrate",palette="npg")
dev.off()
...
head(res.cat)
diff<-survdiff(Surv(futime, fustat)~Carbohydrate, data = res.cat)
...
outFile="F:/Ashley_Projects/Metabolism_Immune/01.IdentificationOfKeyMetabolicPathways/02_Carbohydrate/Carbohydrate_survival.pdf"
