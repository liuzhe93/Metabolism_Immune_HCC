setwd("F:/Ashley_Projects/Metabolism_Immune/05.GC_TME/02_Heatmap28immunecells/")
rm(list=ls())
...
mydata_cluster<-read.csv("F:/Ashley_Projects/Metabolism_Immune/04.MRGsClusters/04_MRG_clusters/Sample_Cluster.csv")
head(mydata_cluster)
dim(mydata_cluster)
#[1] 390   2
...
merged_clu_imm$Id<-substring(merged_clu_imm$X, 1, 12)
head(merged_clu_imm)

merged_clu_imm$Id<-gsub("\\.", "\\-", merged_clu_imm$Id)


merged_data<-merge(sur_time, merged_clu_imm, by = "Id")
dim(merged_data)
#[1] 355  43
merged_data_sel<-subset(merged_data, select = c("X", "cluster", "futime", "fustat", 
...
merged_srt<-merged_data_sel[order(merged_data_sel[,"cluster"]),]
rownames(merged_srt)<-merged_srt$X
...
data_label<-merged_srt_t[c("cluster", "futime", "fustat", "age", "gender", "stage", "T", "M", "N"),]
data_label[1:9,1:3]
...
annotation_col$sample_subtypes<-paste0("C", annotation_col$cluster)
