...
merged<-merge(GC_data, ic50, by.x = "ID", by.y = "sample_name")
dim(merged)
#[1] 480  63
...
####################################drug15： GSK269962A################################################
drug_sen_GSK269962A<-cbind(merged_format$MC,as.numeric(merged_format$GSK269962A))
rownames(drug_sen_GSK269962A)<-row.names(merged_format)
colnames(drug_sen_GSK269962A)<-c("Risk","IC50")
head(drug_sen_GSK269962A)
drug_sen_GSK269962A<-as.data.frame(drug_sen_GSK269962A)
drug_sen_GSK269962A$IC50<-as.numeric(drug_sen_GSK269962A$IC50)
library(ggpubr)
pdf("GSK269962A.pdf")
p <- ggboxplot(drug_sen_GSK269962A, x="Risk",
               y = "IC50", color = "Risk",
               palette = "jco", add = "jitter")
# 添加p值
p + stat_compare_means()
...
####################################drug20： AUY922################################################
drug_sen_AUY922<-cbind(merged_format$MC,as.numeric(merged_format$AUY922))
rownames(drug_sen_AUY922)<-row.names(merged_format)
colnames(drug_sen_AUY922)<-c("Risk","IC50")
head(drug_sen_AUY922)
drug_sen_AUY922<-as.data.frame(drug_sen_AUY922)
drug_sen_AUY922$IC50<-as.numeric(drug_sen_AUY922$IC50)
library(ggpubr)
pdf("AUY922.pdf")
p <- ggboxplot(drug_sen_AUY922, x="Risk",
               y = "IC50", color = "Risk",
               palette = "jco", add = "jitter")
# 添加p值
p + stat_compare_means()
...
####################################drug34： Bosutinib################################################
drug_sen_Bosutinib<-cbind(merged_format$MC,as.numeric(merged_format$Bosutinib))
rownames(drug_sen_Bosutinib)<-row.names(merged_format)
colnames(drug_sen_Bosutinib)<-c("Risk","IC50")
head(drug_sen_Bosutinib)
drug_sen_Bosutinib<-as.data.frame(drug_sen_Bosutinib)
drug_sen_Bosutinib$IC50<-as.numeric(drug_sen_Bosutinib$IC50)
library(ggpubr)
pdf("Bosutinib.pdf")
p <- ggboxplot(drug_sen_Bosutinib, x="Risk",
               y = "IC50", color = "Risk",
               palette = "jco", add = "jitter")
# 添加p值
p + stat_compare_means()
